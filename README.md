# Akimbo — Dual Monitor Workstation for World of Warcraft

Akimbo places the 3D world on a calibrated game viewport and provides a secondary
...
Copy the addon into `World of Warcraft/_classic_era_/Interface/AddOns/Akimbo`.
Enable it in the addon list. Restart WoW after first installation; use `/reload`
after updating existing Lua files.

Set WoW to Windowed mode and run the included `Akimbo-Span.bat` or
`Akimbo-Span.ps1` companion. The companion sizes the window to the Windows virtual
...
`/akimbo bottom <pixels>` remains available.

Global UI size is based on the game viewport. Akimbo applies one baseline to
...
scale is restored when Akimbo is disabled; after reload with Akimbo disabled, WoW
...
| Command | Effect |
| --- | --- |
| `/akimbo`, `/ak`, `/akimbo wizard`, `/akimbo setup` | Open the calibration wizard. |
| `/akimbo settings` or `/akimbo options` | Open detailed settings, including bezel gap and module toggles. |
| `/akimbo guide` or `/akimbo span` | Open the window-spanning guide. |
| `/akimbo seam 36` | Set deck width to 36% of the whole window; supported range 15–80%. |
| `/akimbo hud 70` | Set global UI size to 70% relative to the game viewport; range 25–125%. |
| `/akimbo bottom 6` | Place the game bottom six physical pixels above the canvas bottom. |
| `/akimbo 169` or `/akimbo 219` | Select 16:9 or 21:9 game aspect ratio. |
| `/akimbo ar 1.6` | Select a custom aspect ratio. |
| `/akimbo fill` | Use the configured game-height fraction instead of a fixed aspect ratio. |
| `/akimbo height 56.25` | Set game height to 56.25% of canvas height, used only in Fill mode (5–100%). |
| `/akimbo chat game` or `/akimbo chat deck` | Place managed chat in the game view or workspace. |
| `/akimbo diag` | Report physical viewport bounds and save a geometry snapshot. |
| `/akimbo apply` | Reapply the complete layout. |
| `/akimbo toggle` | Toggle Akimbo. |
| `/akimbo reset` | Reset saved calibration and options to defaults, including zero bottom inset. |
| `/akimbo debug` | Toggle diagnostic chat output. |
...
`Akimbo-Companion.bat` opens the GUI/tray controller. `Akimbo-Span.bat` spans once;
`Akimbo-Watcher.bat` monitors subsequent game launches. All use the shared
`Companion/Akimbo-Window.ps1`, which must be included when distributing the scripts.
...
verify the current character's live enabled state: enable Akimbo in WoW yourself.
...
With **Independent, resizable World Map on Deck** enabled, Akimbo removes only the
