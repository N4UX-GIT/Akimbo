...
function Test-Path { param($LiteralPath,$PathType) return $LiteralPath.EndsWith('Akimbo_Vanilla.toc') }
$fake = [pscustomobject]@{Id=42;MainWindowHandle=[IntPtr]99;MainModule=@{FileName='C:\AkimboTest\WowClassic.exe'}}
$fake | Add-Member ScriptMethod Refresh {}
$status = Test-AkimboAddonStatus $fake
Assert $status.Installed 'Vanilla TOC not recognized'
Assert ($null -eq $status.Enabled) 'Installation was misreported as enabled'
Assert (-not (Test-AkimboAddonStatus $null).Installed) 'Absent client passed gate'
$unreadable = [pscustomobject]@{MainModule=$null}
Assert (-not (Test-AkimboAddonStatus $unreadable).Installed) 'Unreadable client passed gate'
$bounds = Invoke-AkimboSpan $fake
Assert ($bounds.X -eq -1440 -and $bounds.Y -eq -1114 -and $bounds.Width -eq 4000 -and $bounds.Height -eq 2560) 'Incorrect span geometry'
Assert ([AkimboNative]::Restores -eq 1) 'Maximized window not restored'
Assert ([AkimboNative]::Style -eq 0) 'Borders not removed'
Assert ([AkimboNative]::Dpi -eq [IntPtr]1) 'DPI context not restored'
[AkimboNative]::Minimized=$true
[void](Invoke-AkimboSpan $fake)
Assert ([AkimboNative]::Restores -eq 2) 'Minimized window not restored'
$oldRect=[AkimboNative]::Current
[AkimboNative]::Style=0x00C40000
[AkimboNative]::Reject=$true
$failed=$false
try { Invoke-AkimboSpan $fake } catch { $failed=$true }
Assert $failed 'Rejected span reported success'
Assert ([AkimboNative]::Style -eq 0x00C40000 -and [AkimboNative]::Current.Left -eq $oldRect.Left) 'Failed span not rolled back'
Assert ([AkimboNative]::Dpi -eq [IntPtr]1) 'Failed span leaked DPI context'
[AkimboNative]::Owner=999
$moves=[AkimboNative]::Moves
$failed=$false
try { Invoke-AkimboSpan $fake } catch { $failed=$true }
Assert ($failed -and [AkimboNative]::Moves -eq $moves) 'Wrong-process window was moved'
